...
from image_synthesis.modeling.codecs.image_codec.spatial_transform import LearnableSpatialTransformWrapper
from image_synthesis.modeling.codecs.image_codec.squeeze_excitation import SELayer
...
class PatchEncoder2(nn.Module):
    def __init__(self, *, 
        ...
        super().__init__()
        # patch size: stride * stride channel:in_ch
...
class PatchConvDecoder2(nn.Module):
    def __init__(self, *, 
        ...
        cur_conv_kwargs = {'ratio_gin': 0, 'ratio_gout': 0.75, 'enable_lfu': False}
        init_conv_kwargs = {'ratio_gin': 0, 'ratio_gout': 0, 'enable_lfu': False}
        self.pre_layers = nn.Sequential(*[
            nn.ReflectionPad2d(3),
            FFC_BN_ACT(input_nc, ngf, kernel_size=7, padding=0, norm_layer=norm_layer,
                       activation_layer=activation_layer, **init_conv_kwargs),
            FFC_BN_ACT(input_nc, ngf, kernel_size=3, stride=1, padding=1, norm_layer=norm_layer,
        ...
        for i in range(num_res_block):
            # res_layers.append(ConvResBlock(res_ch, res_ch//res_block_bottleneck))
            res_layers.append(FFCResnetBlock(feats_num_bottleneck, padding_type='reflect', activation_layer=activation_layer,
        ...
        res_layers.append(ConcatTupleLayer())
